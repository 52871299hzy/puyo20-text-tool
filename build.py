...
def build_image(filename, data, size):
	...
	for i in range (Line):
		for j in range(len(Data[i])):
			text = Data[i][j]
			font = ImageFont.truetype('./NotoSansCJK-Medium.ttc', size)
			# bbox = draw.textbbox((0, 0), text, font=font)
			draw.text(xy=(size * j, size * i - 8), text=text, fill='#FFFFFF', font=font)
			...
			# By the way, it seems that this only applies to some fonts.
...
def build_mtx(filename, text, charmap):
	...
	with open(filename,'wb') as f:
		f.write((header_length + len(section_offsets)*4 + len(line_offsets)*4 + len(data)).to_bytes(4,'little'))
...
def main():
	if len(sys.argv) != 4:
		print("Usage: build.py <input.txt> <output.fnt> <output.mtx>")
		exit(1)
	...
	subprocess.run(['mono','VrConvert.exe','-e','tmp.png','gvr','rgb5a3','rgb5a3','-gcix'])
